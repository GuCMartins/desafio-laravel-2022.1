...
<div>
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-end m-2 p-2">
            <button wire:click="showchampionshipModal">Criar</button>
        </div>
        <div class="m-2 p-2">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @foreach ($championships as $championship)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap flex justify-center">{{ $championship->begin }}/{{ $championship->end }}</td>
                                        <td class="px-6 py-4 text-right text-sm">
                                            <div class="flex space-x-2">
                                                <button class=" px-6" wire:click="showEditchampionshipModal({{ $championship->id }})">Edit
                                                </button>
                                                <button class="bg-red-400 hover:bg-red-600"
                                                    wire:click="deletechampionship({{ $championship->id }})">Delete
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
        ...
        <div>
            <dialog-modal wire:model="showPlayerModal">
                @if ($isEditMode)
                    <x-slot name="title">Atualizar Campeonato</x-slot>
                @else
                    <x-slot name="title">Criar Campeonato</x-slot>
                @endif
                <x-slot name="content">
